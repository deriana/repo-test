...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Tambah Disposisi Surat: {{ $letter->reference_number }}</h1>
        </div>

        <div class="section-body">
            <div class="card">
                <form action="{{ route('transaction.disposition.store', $letter->id) }}" method="POST">
                    @csrf
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="letter_status" class="form-control select2" required>
                                ...
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id }}" {{ old('letter_status') == $status->id ? 'selected' : '' }}>
                                        {{ $status->status }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="card-footer text-right">
                        <a href="{{ route('transaction.disposition.index', $letter->id) }}" class="btn btn-secondary">Batal</a>
